...
let to_binary_list (n : int) : int list2 = 
	let rep = {first = None; last = None} in
	let rec aux n =
		if n <> 0 then begin
			setfirst rep (n mod 2);
			aux (n/2)
		end in 
	aux n;
	rep
...
let _ = 
	...
	for i = 1 to 1000000 do
		if palindrome_number i && palindrome_list (to_binary_list i) then begin 
			rep := !rep +i
		end
	done;
	Printf.printf "%d\n" !rep